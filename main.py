...
async def convert_svg_to_pes(file: UploadFile = File(...)):
    ...
    try:
        ...
        # --- CAMBIO CRÍTICO AQUÍ ---
        # Inkstitch requiere que los argumentos usen el formato --flag=valor
        # Si se separan por espacio, lanza el ValueError que vimos antes.
        command = [
            "inkstitch", 
            f"--output={output_path}",  # Unido con '='
            input_path
        ]
        ...
        if process.returncode != 0:
            # Capturamos el error específico para debuguear
