...
# 2. Instalar Inkstitch CLI (Extracción y localización automática)
RUN curl -L -f "https://github.com/inkstitch/inkstitch/releases/download/v3.0.1/inkstitch-3.0.1-linux.tar.xz" -o inkstitch.tar.xz \
    ...
    && INK_PATH=$(find /usr/local/bin/inkstitch_cli -type f -name "inkstitch" | head -n 1) \
    && chmod +x "$INK_PATH" \
    && ln -sf "$INK_PATH" /usr/local/bin/inkstitch \
    && rm inkstitch.tar.xz
