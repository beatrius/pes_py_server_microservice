...
RUN curl -L -f "https://github.com/inkstitch/inkstitch/releases/download/v3.0.1/inkstitch-3.0.1-linux.tar.xz" -o inkstitch.tar.xz \
    ...
    && chmod +x /usr/local/bin/inkstitch_cli/inkstitch/inkstitch \
    && ln -s /usr/local/bin/inkstitch_cli/inkstitch/inkstitch /usr/local/bin/inkstitch \
    && rm inkstitch.tar.xz
